...
H11 & Note sulla ricerca & Biblioteca Proibita & Distingue l'ipotesi del contatto dai fatti osservati. \\
...
H16 & Rapporto sul Progetto & Il Progetto & Rivela obiettivo, limiti e prelievi senza consenso. \\
...
\textbf{Nota di progetto:} il regolatore è un componente da inserire nelle attrezzature mediche. Non è una macchina completa.
...
44-14 & Regolatore di pressione & Laboratorio Alchemico & \textbf{\churchseal\,\churchseal} \\
44-15 & Lastre d'argento e fili sottili & \textit{voce raschiata} & \textbf{\churchseal\,\churchseal} \\
44-16 & Carbone purificato & Laboratorio Alchemico & \churchseal \\
\bottomrule
\end{tabularx}
...
Tre notti fa mi ordinarono di preparare sei pazienti per un «trasferimento terapeutico». Nessuno di loro aveva dato consenso. Nel vano di servizio vidi una vasca coperta, collegata a pompe industriali. All'interno c'era tessuto vivo; la macchina faceva circolare sangue nella vasca. Non so quale sia lo scopo della coltura.
...
% H11
% ----------------------------------------------------------------
\newhandout{H11}{Note sulla ricerca}{Appunti della dottoressa Miriam Voss}
\begin{documentbox}
\handfont
\textbf{Dopo il colloquio con Elias Ward}

Ward riferisce che Silas ha venduto alla Chiesa una copia dei suoi disegni. Ha seguito fino al Laboratorio Alchemico una cassa la cui ricevuta dichiara un regolatore. Non ha visto il contenuto: teme che sia stato riprodotto il suo progetto, ma non può ancora confermarlo.

Il sigillo richiama simboli pthumeriani legati a nascita e legame. Nei testi il cordone di un Grande Essere viene associato alla possibilità di un rapporto con esseri superiori.

\textbf{Ipotesi:} la cerchia potrebbe cercare di coltivare qualcosa capace di svolgere una funzione simile. Non è la stessa cosa che creare un Grande Essere.

La funzione del regolatore è mantenere stabile la pressione. Se serve a una coltura biologica, spiega come questa venga sostenuta, non come si possa ottenere un contatto.

\medskip
Non abbiamo visto un contatto né trovato una procedura che lo garantisca. Occorre distinguere lo scopo della ricerca dai risultati.
...
% H15
% ----------------------------------------------------------------
\newhandout{H15}{Schema della macchina}{Tavola tecnica del laboratorio inferiore}
\begin{bluebox}
\centering
\begin{tikzpicture}[x=1cm,y=1cm,font=\small,>=Latex]
\draw[ink,line width=.8pt] (-5,-1.5) rectangle (5,3);
\node[anchor=west,font=\bfseries] at (-4.7,2.6) {MACCHINA};
\node[align=center] (p) at (-3,1) {pompe};
\node[draw=ink,minimum width=3cm,minimum height=1cm,align=center] (r) at (1,1) {REGOLATORE\\pressione stabile};
\draw[->,thick] (p)--(r);
\node[draw=blood,ellipse,minimum width=4cm,minimum height=1.3cm,align=center] (o) at (0,-3.5) {ORGANISMO\\nella vasca};
\draw[->,thick] (r.south)--(1,-1)--(o.north east);
\draw[->,thick] (o.north west)--(-3,-1)--(p.south);
\end{tikzpicture}

\medskip
\raggedright
La macchina mantiene vivo l'organismo facendo circolare il sangue.
Il regolatore mantiene stabile la pressione.

\textit{Nota di Elias:} «Il regolatore riproduce il mio progetto. La macchina è stata costruita attorno a esso per un impiego che non avevo previsto.»
\end{bluebox}
...
\begin{churchbox}
\begin{center}
\churchseal\quad {\Large\bfseries STATO DELLA RICERCA}\quad\churchseal
\end{center}

\textbf{Organismo:} coltura artificiale ottenuta da tessuti ospedalieri e materiale biologico pthumeriano.

\textbf{Macchina:} sostiene la coltura mediante circolazione del sangue e controllo delle condizioni di mantenimento.

\textbf{Regolatore:} componente riprodotto dai disegni di Elias Ward. Mantiene stabile la pressione durante la circolazione.

\textbf{Obiettivo:} sviluppare un organismo capace di consentire un contatto con un'entità ultraterrena, imitando una funzione attribuita al cordone di un Grande Essere.

\textbf{Risultato attuale:} l'organismo viene mantenuto vivo. Nessun contatto è stato verificato; non sono stabiliti né un metodo certo né una data per ottenerlo.

\textbf{Materiale biologico:} sangue e tessuti prelevati ai pazienti dell'Ospedale. Autorizzazione della direzione; consenso individuale non acquisito.

\textbf{Disposizione:} proseguire la ricerca. Mantenere separati dai registri pubblici i documenti relativi ai prelievi e alle sperimentazioni.
...
\handfont
\large
Il regolatore è il mio. Non un oggetto simile: riconosco il progetto.

La macchina che hanno costruito lo usa per mantenere vivo ciò che si trova nella vasca. L'organismo non è la macchina, e nessuno dei due è una mia invenzione.

Non so se possa davvero permettere il contatto che cercano. So che stanno usando il mio lavoro e materiale preso ai malati senza consenso.
...
\medskip
Non confondete ciò che la macchina fa con ciò che Celestine spera di ottenere. Non sono la stessa cosa.
